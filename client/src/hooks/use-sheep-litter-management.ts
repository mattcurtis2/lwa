...
import { SheepLitter, Sheep } from "@db/schema";
import { formatApiDate } from "@/lib/date-utils";

interface ExtendedSheepLitter extends SheepLitter {
  mother?: Sheep;
  father?: Sheep;
  lambs?: Sheep[];
...
export function useSheepLitterManagement() {
  const { toast } = useToast();
  const [showLitterForm, setShowLitterForm] = useState(false);
  const [litterFormMode, setLitterFormMode] = useState<'create' | 'edit'>('create');
  const [editLitter, setEditLitter] = useState<ExtendedSheepLitter | null>(null);
  ...
  const createLitter = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/sheep-litters", {
        method: "POST",
        headers: {
  ...
  const updateLitter = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/sheep-litters/${editLitter.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
