...
export default function Admin() {
  ...
  const createDog = useMutation({
    mutationFn: async (dog: Partial<Dog>) => {
      ...
      if (!res.ok) {
        const error = await res.text();
        throw new Error(error || 'Failed to create dog');
      }
    ...
    onError: (error: Error) => {
      toast({
        title: "Error",
        description: error.message || 'Failed to create dog',
        variant: "destructive",
      });
    }
  });
  ...
  const updateDog = useMutation({
    mutationFn: async (dog: Partial<Dog>) => {
      ...
      if (!res.ok) {
        const error = await res.text();
        throw new Error(error || 'Failed to update dog');
      }
    ...
    onError: (error: Error) => {
      toast({
        title: "Error",
        description: error.message || 'Failed to update dog',
        variant: "destructive",
      });
    }
  });
  ...
  const handleDogFormSubmit = async (values: Partial<Dog>) => {
    if (values.id) {
      await updateDog.mutateAsync(values);
    } else {
      await createDog.mutateAsync(values);
    }
  };
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Add and manage dogs</CardDescription>
                </div>
                <Button onClick={handleAddNewDog}>
                  <Plus className="h-4 w-4 mr-2" />
                  Add New Dog
                </Button>
              </div>
