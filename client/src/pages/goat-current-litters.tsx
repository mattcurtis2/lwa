      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid gap-8">
          {sortedLitters.map((litter) => (
            ...
            >
              {/* Compact banner with parent and kid images */}
                      ...
                      <div className="flex items-center gap-1 ml-4 flex-wrap">
                        {litter.kids.slice(0, 4).map((kid) => (
                          <div key={kid.id} className="flex flex-col items-center">
                            <div className={`w-12 h-12 rounded-full overflow-hidden ${kid.gender === 'female' ? 'bg-pink-50 border-pink-100' : 'bg-blue-50 border-blue-100'} border-2 flex-shrink-0`}>
                              {kid.profileImageUrl ? (
                                <img 
                        ...
                        {litter.kids.length > 4 && (
                          <div className="flex items-center justify-center w-12 h-12 rounded-full bg-amber-100 border-2 border-amber-200">
                            <span className="text-xs font-semibold text-amber-800">+{litter.kids.length - 4}</span>
                          </div>
                        )}
                      </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          {litter.kids.slice(0, 4).map((kid) => (
                            <div key={kid.id} className="flex items-center gap-2">
                              <div className={`w-6 h-6 rounded-full overflow-hidden ${kid.gender === 'female' ? 'bg-pink-50 border-pink-100' : 'bg-blue-50 border-blue-100'} border flex-shrink-0`}>
                                {kid.profileImageUrl ? (
                                  <img 
                                    src={kid.profileImageUrl} 
                                    alt={kid.name}
                                  ...
                                  <div className="w-full h-full flex items-center justify-center">
                                    <span className={`text-xs ${kid.gender === 'female' ? 'text-pink-400' : 'text-blue-400'}`}>
                                      {kid.gender === 'female' ? '♀' : '♂'}
                                    </span>
                                  </div>
                                )}
