...
async function migrateDogsToS3() {
  try {
    console.log('Starting dog media migration to S3...');
    ...
    console.log('\n=== Dog migration completed successfully ===');
  } catch (error) {
    console.error('Dog migration failed with error:', error);
  }
}

// Run the migration
migrateDogsToS3().then(() => {
  console.log('Dog migration script execution completed');
  process.exit(0);
}).catch(error => {
  console.error('Unhandled error in dog migration script:', error);
  process.exit(1);
});
